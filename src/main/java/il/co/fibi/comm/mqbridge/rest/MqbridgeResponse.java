...
public class MqbridgeResponse {
	@Inject @Cache(CacheType.CBXML) ICache cbxml;
	@Inject MainframeConvertor convertor;
	...
	public MqbridgeResponse fromServiceResponse(String trxid, ServiceResponse resp) {
		...
		if (status == 0) {
			...
			if (resp.getData() != null) {
				String resid = new String(resp.getData().substring(0, 2).getBytes(StandardCharsets.ISO_8859_1), CharsetICU.forName("Cp037"));
				String data = convertor.data2xml(resp.getData(), (String) cbxml.get(trxid.concat("_").concat(resid)));
				res = XML.toJSONObject(data, true).getJSONObject("copybook").getJSONObject(trxid.concat("_").concat(resid));
			} else {
				res = new JSONObject();
			}
